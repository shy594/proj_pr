...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/"
...
#3、跑命令
{
  ...
  cat("#!/bin/bash\n", file = shell)
  cmd_01 <- paste0("#cp ", path,"rawdata/RawData/*/*.fq.gz ", path, "fq/", "\n")
  cmd_02 <- paste0("#rename s/E14-// ", path, "fq/*gz", "\n")
  prex <- gsub(pattern = "E14-", replacement = "", x = list.files((paste0(path,"rawdata/RawData"))))
  ...
  cmd_07 <- paste0("bamCompare -p 20 --scaleFactorsMethod SES --sampleLength 100000 -b1 ",
                   paste0(path,"bam/csem/",prex[5:8],".sorted.bam -b2 "),
                   paste0(path,"bam/csem/",prex[c(1,3,2,4)],".sorted.bam -o "),
                   paste0(path,"bw/bcp/",prex[5:8],"VSinput.bw"),"\n")
  ...
  cmd_13 <- paste0("macs2 callpeak -f BAMPE -g mm -p 0.05 --keep-dup 1 -t ",
                   path,"bam/csem/",prex[5:8],".sorted.bam -c ",
                   path,"bam/csem/",prex[c(1,3,2,4)],".sorted.bam -n ",
                   prex[5:8],"-N --outdir ",
                   path,"peak/", "\n")
  cmd_14 <- paste0("macs2 callpeak -f BAMPE -g mm -p 0.05 --broad --broad-cutoff 0.05 ",
                   "--keep-dup 1 -t ",path,"bam/csem/", prex[5:8],
                   ".sorted.bam -c ", path,"bam/csem/", prex[c(1,3,2,4)],
                   ".sorted.bam -n ", prex[5:8],
                   "-B --outdir ", path,"peak/", "\n")
  cmd_15 <- paste0("#TEtranscripts -t ",
                   paste0(path,"bam/csem/",prex[5:8],".sorted.bam", collapse = " ")," -c ",
                   paste0(path,"bam/csem/",prex[c(1,3,2,4)],".sorted.bam", collapse = " "),
  ...
  cmd_18 <- paste0("run-csem -p 20 --no-extending-reads --bam ",
                   paste0(path,"mervl/",prex,".bam")," 140 ",
                   paste0(path,"mervl/csem/",prex),"\n")
  cmd_19 <- paste0("bamCoverage --normalizeUsing CPM -p 20 --minMappingQuality 1 ",
                   "--samFlagExclude 256 -of bigwig -bs 20 -b ",
                   paste0(path,"mervl/csem/",prex,".sorted.bam -o "),
                   paste0(path,"mervl/",prex,".mervl.bw\n"))
...
#
for (i in (prex[5:8])) {
  pk01 <- suppressWarnings(ChIPseeker::annotatePeak(peak = paste0(path, "peak/",i,"-B_peaks.broadPeak"), 
                                                    tssRegion = c(-2000, 2000), TxDb = txb1))
  pdf(file = paste0(path, "PLOT/",i,"-B_chipseekerAnno.pdf"), width = 8, height = 8)
  ChIPseeker::plotAnnoPie(pk01, legend.position = "rightside")
  dev.off()
  write.csv(as.data.frame(pk01), paste0(path, "peak/",i,"-B_chipseekerAnno.csv"), quote = F, row.names = F)
}
##
##
##
...
colnames(red1) <- prex[c(5:8,1,3,2,4)]
...
for (m in seq(4)) {
  ...
  openxlsx::write.xlsx(x = red3, asTable = T, file = paste0(path,"fisher/",prex[5:8][m],"FisherTE.xlsx"))
}
##
##
##
...
for (m in seq(4)) {
  ...
  openxlsx::write.xlsx(x = red3, asTable = T, file = paste0(path,"fisher/",prex[5:8][m],"FisherGene.xlsx"))
}
##
##
##
...
shy_diff(mydata = rc_a[,c(1:3,6,7)], id = "prGene", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_b[,c(1:3,6,7)], id = "prTE", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_c[,c(1,6,7,2,4)], id = "prTEsites", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_a[,c(1,4,5,8,9)], id = "pp8Gene", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_b[,c(1,4,5,8,9)], id = "pp8TE", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_c[,c(1,8,9,3,5)], id = "pp8TEsites", Class = factor(c("IP","IP","WT","WT")))
...
i = c("test","input")
m = c("pr-1","Input-1-P1")
shell <- paste0("/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/run_x.sh")
cat("#!/bin/bash\n", file = shell)
cmd_01 <- paste0("#fastp -i /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/fq/",
                 m, "_1.fq.gz -I /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/fq/",
                 m, "_2.fq.gz -f 10 -t 1 -F 10 -T 1 -g -x -n 5 -l 20 --detect_adapter_for_pe ",
                 "-m --merged_out /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/",
                 i, ".fq.gz","\n")
cmd_02 <- paste0("#bowtie2 -q --sensitive -k 5 --reorder -p 20 -x ",inx1, " -U ",
                 "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/",i,".fq.gz |",
                 "samtools view -F 4 -b > ",
                 "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/",i,".bam","\n")
cmd_03 <- paste0("#run-csem -p 20 --no-extending-reads --bam ",
                 "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/",i,
                 ".bam ",240, " /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/",
                 i,"CSEM","\n")
#
cmd_04 <- paste0("bamCompare -p 20 --scaleFactorsMethod readCount -b1 ",
                 "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testCSEM.sorted.bam ",
                 "-b2 /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/inputCSEM.sorted.bam ",
                 "-o /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testvsinpu.bw","\n")
cmd_05 <- paste0("computeMatrix scale-regions -p 20 -a 2000 -b 2000 -m 5000 ",
                 "--missingDataAsZero -p 20 -R ", inx7, " ",
                 "-S /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testvsinpu.bw ",
                 "-o /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testvsinpu.mat.gz","\n")
cmd_06 <- paste0("plotHeatmap --heatmapHeight 12 --colorList white,red -m ",
                 "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testvsinpu.mat.gz ",
                 "-out /ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/testvsinpu6.pdf","\n")
for (i in cmd_01) {cat(i, append = T, file = shell)}
for (i in cmd_02) {cat(i, append = T, file = shell)}
for (i in cmd_03) {cat(i, append = T, file = shell)}
for (i in cmd_04) {cat(i, append = T, file = shell)}
for (i in cmd_05) {cat(i, append = T, file = shell)}
for (i in cmd_06) {cat(i, append = T, file = shell)}
print(paste0("nohup bash ",shell," >/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/pairEND/dumpROOM/run_x.log"," 2>&1 &"))
}
