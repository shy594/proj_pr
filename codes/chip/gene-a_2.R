...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231109/"
...
#3、跑命令
{
  for (i in c("src","fq","Log","peak","rRNA","rRNA/csem","mervl","mervl/csem","fisher","deeptools","bam","bam/csem","trim","PLOT","bw/bcv","bw/bcp","count")) {
  ...
  cmd_17 <- paste0("bowtie2 -q --very-sensitive -k 5 --reorder -p 20 -x ",
                   inxC," --no-mixed --no-discordant -1 ",lay1," -2 ",lay2,
                   " |samtools view -F 4 -b > ", path,
                   "rRNA/",prex,".bam","\n")
  cmd_18 <- paste0("run-csem -p 20 --no-extending-reads --bam ",
                   paste0(path,"rRNA/",prex,".bam")," 140 ",
                   paste0(path,"rRNA/csem/",prex),"\n")
  cmd_19 <- paste0("bamCoverage --normalizeUsing CPM -p 20 --minMappingQuality 1 ",
                   "--samFlagExclude 256 -of bigwig -bs 20 -b ",
                   paste0(path,"rRNA/csem/",prex,".sorted.bam -o "),
                   paste0(path,"rRNA/",prex,".rRNA.bw\n"))
