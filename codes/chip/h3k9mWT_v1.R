...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231120/H3K9me3/mWT/"
...
#3、跑命令
{
  ...
  cat("#!/bin/bash\n", file = shell)
  prex <- c("mWT-IP-1","mWT-IP-2","mWT-Input-1","mWT-Input-2")#; unique(sapply(str_split(list.files(path = paste0(path, "rawdata")), pattern = "74-|_"), "[", 2))
