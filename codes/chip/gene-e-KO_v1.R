...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231109/Kap1KO/"
...
#3、跑命令
{
  ...
  cat("#!/bin/bash\n", file = shell)
  prex <- sapply(str_split(list.files(path = paste0(path, "rawdata")), pattern = "KO-"), "[", 2)
...
shy_diff(mydata = rc_a, id = "Kap1KOGene", Class = factor(c("IP","IP","IP","WT","WT","WT")))
shy_diff(mydata = rc_b, id = "Kap1KOTE", Class = factor(c("IP","IP","IP","WT","WT","WT")))
shy_diff(mydata = rc_c[,c(1,5:7,2:4)], id = "Kap1KOTEsites", Class = factor(c("IP","IP","IP","WT","WT","WT")))
##
##
##
################################################################################
