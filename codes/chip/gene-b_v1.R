...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231109/ff/"
  ...
  inxE = "/Reference/aaSHY/Ensembl99/GRCm38/BED/TEsubFamily/RLTR1B-int::ERV1::LTR.bed"
}
##
##
##
...
#3、跑命令
{
  ...
  prex <- sapply(str_split(list.files(path = paste0(path, "rawdata")), pattern = "E14-"), "[", 2)
...
#suppl
{
  cmd_11 <- paste0("computeMatrix scale-regions -p 20 -a 1000 -b 1000 -m 3000 ",
                   "--missingDataAsZero -p 20 -R ", inxE, " -S ",
                   paste0(list.files(paste0(path, "bw/bcv"), full.names = T), collapse = " "),
                   " -o ",path,"deeptools/",basename(inxE),".mat.gz","\n")
  cmd_12 <- paste0("plotHeatmap --heatmapHeight 12 --perGroup --colorList white,red -m ",
                   path,"deeptools/",basename(inxE),".mat.gz -out ",
                   path,"PLOT/",basename(inxE),".pdf","\n")
}
##
##
##
...
shy_diff(mydata = rc_a[,1:5], id = "ffGene", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_b[,1:5], id = "ffTE", Class = factor(c("IP","IP","WT","WT")))
shy_diff(mydata = rc_c[,1:5], id = "ffTEsites", Class = factor(c("IP","IP","WT","WT")))
##
##
##
################################################################################
