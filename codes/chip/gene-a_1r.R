...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231108/singleEND/"
...
#3、跑命令
{
  for (i in c("src","fq","Log","peak","DESeq2","rRNA/csem","mervl/csem","fisher","deeptools","bam/csem","trim","PLOT","bw/bcv","bw/bcp","count")) {
  ...
  cat("#!/bin/bash\n", file = shell)
  prex <- sapply(str_split(list.files(path = paste0(path, "../pairEND/trim"), pattern = "_1.fq.gz$"), pattern = "_1"), "[", 1)
  lay1 <- paste0(path,"../pairEND/fq/",prex,"_1.fq.gz")
  lay2 <- paste0(path,"../pairEND/fq/",prex,"_2.fq.gz")
  lay3 <- paste0(path,"dumpROOM/",prex,"_1.fq.gz")
  lay4 <- paste0(path,"dumpROOM/",prex,"_2.fq.gz")
  cmd_01 <- paste0("#zcat ", c(lay1, lay2), " |sed 's/^@/@",c(rep(1,8),rep(2,8)),"/g' |gzip >", c(lay3,lay4), "\n")
...
wr01 <- cpm2[,c(5,9,6,10,7,11,8,12)-4]
#与MERVL RNA互作的2C基因
...
df03 <- df03[,c(1:4,5,9,6,10,7,11,8,12,c(5,9,6,10,7,11,8,12)+8)]
