...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/ChIPseq/20231109/Kap1WT/"
...
##
#质检
shell <- paste0(path,"src/run_x.sh")
cat("#!/bin/bash\n", file = shell)
cmd_01 <- paste0("fastqc -t 10 -o ", path, "fq/ ", list.files(path = paste0(path, "fq"), pattern = "*fq.gz$", full.names = T),"\n")
for (i in cmd_01) {cat(i, append = T, file = shell)}
print(paste0("nohup bash ",shell," >",path,"Log/run_x.log"," 2>&1 &"))
##
##
##
#3、跑命令
{
  ...
  cat("#!/bin/bash\n", file = shell)
  prex <- sapply(str_split(list.files(path = paste0(path, "rawdata")), pattern = "WT-"), "[", 2)
  ...
  cmd_01 <- paste0("zcat ", c(lay1, lay2), " |sed 's/^@/@",c(rep(1,6),rep(2,6)),"/g' |gzip >", c(lay3,lay4), "\n")
  cmd_02 <- paste0("zcat ",lay3, " ", lay4," |gzip >",paste0(path,"fq/",prex,".fq.gz"),"\n")
  cmd_03 <- paste0("#fastp -i ", path, "fq/",prex,".fq.gz -f 10 -t 1 -g -x -Q -D -n 5 -l 20 -o ",path,
                   "trim/", prex, ".fq.gz","\n")
  ...
  cmd_07 <- paste0("bamCompare -p 20 --scaleFactorsMethod SES --sampleLength 100000 -b1 ",
                   paste0(path,"bam/csem/",prex[4:6],".sorted.bam -b2 "),
                   paste0(path,"bam/csem/",prex[1:3],".sorted.bam -o "),
                   paste0(path,"bw/bcp/",prex[4:6],"VSinput.bw"),"\n")
  for (i in cmd_01) {cat(i, append = T, file = shell)}
  ...
  cmd_13 <- paste0("macs2 callpeak -f BAM -g mm -q 0.05 --keep-dup 1 -t ",
                   path,"bam/csem/",prex[4:6],".sorted.bam -c ",
                   path,"bam/csem/",prex[1:3],".sorted.bam -n ",
                   prex[4:6],"-N --outdir ",
                   path,"peak/", "\n")
  cmd_14 <- paste0("macs2 callpeak -f BAM -g mm -q 0.05 --broad --broad-cutoff 0.05 ",
                   "--keep-dup 1 -t ",path,"bam/csem/", prex[4:6],
                   ".sorted.bam -c ", path,"bam/csem/", prex[1:3],
                   ".sorted.bam -n ", prex[4:6],
                   "-B --outdir ", path,"peak/", "\n")
  cmd_15 <- paste0("TEtranscripts -t ",
                   paste0(path,"bam/csem/",prex[4:6],".sorted.bam", collapse = " ")," -c ",
                   paste0(path,"bam/csem/",prex[1:3],".sorted.bam", collapse = " "),
...
#
for (i in (prex[4:6])) {
  fhao <- list(c("B","broad"),c("N","narrow"))
  for (m in seq(2)) {
    if (paste0(i, "-", fhao[[m]][1], "_peaks.", fhao[[m]][2], "Peak") %in% list.files(path = paste0(path, "peak"))) {
      pk01 <- suppressWarnings(ChIPseeker::annotatePeak(peak = paste0(path, "peak/", i, "-", fhao[[m]][1], "_peaks.", fhao[[m]][2], "Peak"), 
                                                        tssRegion = c(-2000, 2000), TxDb = txb1))
      pdf(file = paste0(path, "PLOT/",i,"-",fhao[[m]][1],"_chipseekerAnno.pdf"), width = 8, height = 8)
      ChIPseeker::plotAnnoPie(pk01, legend.position = "rightside")
      dev.off()
      write.csv(as.data.frame(pk01), paste0(path, "peak/",i,"-",fhao[[m]][1],"_chipseekerAnno.csv"), quote = F, row.names = F)
    }
  }
}
##
...
for (m in seq(3)) {
  ...
  openxlsx::write.xlsx(x = red3, asTable = T, file = paste0(path,"fisher/",prex[4:6][m],"FisherTE.xlsx"))
}
##
##
##
...
for (m in seq(3)) {
  ...
  openxlsx::write.xlsx(x = red3, asTable = T, file = paste0(path,"fisher/",prex[4:6][m],"FisherGene.xlsx"))
}
##
##
##
...
shy_diff(mydata = rc_a, id = "Kap1WTGene", Class = factor(c("IP","IP","IP","WT","WT","WT")))
shy_diff(mydata = rc_b, id = "Kap1WTTE", Class = factor(c("IP","IP","IP","WT","WT","WT")))
shy_diff(mydata = rc_c[,c(1,5:7,2:4)], id = "Kap1WTTEsites", Class = factor(c("IP","IP","IP","WT","WT","WT")))
##
##
##
################################################################################
...
df03 <- df03[,c(1:4,6,9,7,10,c(6,9,7,10)+6)]
openxlsx::write.xlsx(x = df03, file = paste0(path, "mervl/mervlTwoLikeSignal.xlsx"), asTable = T)
#
#
#
wrwr <- cbind(wr01,wr02,wr03,wr04,wr05)
write.table(x = wrwr, quote = F, col.names = T, row.names = T, sep = "\t", file = paste0(path, "../twocChipSignal.txt"))
