...
#2、给索引
{
  path = "/ChIP_seq_2/aaSHY/pr/publicData/TRIM28/"
  inx1 = "/Reference/aaSHY/Ensembl99/GRCm38/INDEX/bowtie2/mm10"
  inx2 = "/Reference/aaSHY/Ensembl99/GRCm38/BED/TEsubFamily/MERVL-int::ERVL::LTR.bed"
  inx3 = "/Reference/aaSHY/Ensembl99/GRCm38/BED/special/twoC.bed"
  inx4 = "/Reference/aaSHY/zOther/Rn45s/INDEX/bowtie2/Rn45s"
...
#2、写命令
{
  prefix <- basename(list.files(paste0(path,"rawdata"), pattern = "*",full.names = T))
  cmd_1 <- paste0("#parallel-fastq-dump -t 20 --split-files --gzip -s ", 
                  paste0(path,"rawdata/",prefix), " -O ",
                  paste0(path,"fq"), "\n")
  cmd_2 <- paste0("#rename s/fastq/fq/ ",path,"fq/*","\n")
  cmd_3 <- paste0("#trim_galore --phred33 --paired -o ",paste0(path,"trim "),
                  paste0(path,"fq/",prefix,"_1.fq.gz "),
                  paste0(path,"fq/",prefix,"_2.fq.gz"),"\n")
  cmd_4 <- paste0("bowtie2 -q -5 3 -3 5 --very-sensitive -k 5 --reorder -p 16 -x ",
                  inx1," --no-mixed --no-discordant -1 ",
                  paste0(path,"trim/",prefix,"_1_val_1.fq.gz -2 "),
                  paste0(path,"trim/",prefix,"_2_val_2.fq.gz"," |samtools view -F 4 -b > "),
                  paste0(path,"bam/",prefix,".bam"),"\n")
  cmd_5 <- paste0("run-csem -p 16 --bam ",
                  paste0(path,"bam/",prefix,".bam")," --no-extending-reads 150 ",
                  paste0(path,"bam/csem/",prefix),"\n")
  cmd_6 <- paste0("bamCoverage --normalizeUsing CPM -p 20 --minMappingQuality 1 --samFlagExclude 256 --outFileFormat bigwig --binSize 20 -b ",
                  paste0(path,"bam/csem/",prefix,".sorted.bam -o "),
                  paste0(path,"bw/bcv/",prefix,".bcv.bw\n"))
  cmd_7 <- paste0("bamCompare -p 20 --scaleFactorsMethod SES --sampleLength 100000 -b1 ",
  ...
  cmd_8 <- paste0("computeMatrix scale-regions -p 30 -a 2000 -b 2000 -m 5000 ",
                  ...
                  paste0(path,"deeptools/",basename(inx2),".mat.gz"),"\n")
  cmd_9 <- paste0("computeMatrix scale-regions -p 30 -a 2000 -b 2000 -m 5000 ",
                  "--missingDataAsZero -p 20 ",
                  "-R ", inx3, " -S ",
                  paste0(path,"bw/bcp/IPvsInput.bw -o "),
                  paste0(path,"deeptools/",basename(inx3),".mat.gz"),"\n")
  cmd_a <- paste0("plotHeatmap --heatmapHeight 12 --colorList white,red -m ",
                  paste0(path,"deeptools/",basename(inx2),".mat.gz")," -out ",
                  paste0(path,"deeptools/heatmap/",basename(inx2),".pdf"),"\n")
  cmd_b <- paste0("plotHeatmap --heatmapHeight 12 --colorList white,red -m ",
                  paste0(path,"deeptools/",basename(inx3),".mat.gz")," -out ",
                  paste0(path,"deeptools/heatmap/",basename(inx3),".pdf"),"\n")
  cmd_c <- paste0("macs2 callpeak -f AUTO -g mm -q 1 --keep-dup auto -t ",
                  paste0(path,"bam/csem/",prefix[2],".sorted.bam -c "),
                  paste0(path,"bam/csem/",prefix[1],".sorted.bam -n TRIM28 --outdir "),
                  paste0(path,"peak/"))
  shell <- paste0("/ChIP_seq_2/aaSHY/pr/publicData/TRIM28/run_a.sh")
  cat("#!/bin/bash\n", file = shell)
...
#
peak <- "/ChIP_seq_2/aaSHY/pr/publicData/TRIM28/peak/TRIM28_peaks.narrowPeak"
x <- annotatePeak(peak, tssRegion=c(-2000, 2000), TxDb=txdb)#as.GRanges(x) %>% head(5)
...
test <- as.data.frame(peakAnno@anno[as.integer(rownames(temp[which(temp$Promoter==T),]))])
